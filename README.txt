...
Myšlenka:
  rozdělení do malých bloků (modulů), které dotají XML a to bude transformováno
    pomocí XSL, cachování XML bloků
  ...
  možnost v rámci jádra získat seznam hash podle nastavení modulů (a jejich
    pravidel pro cache) a poté jedním selectem získat XML pro většinu modulů 
    (bloků) a tím mít stránku velmi rychle.
  použití XSL by rovněž umožnilo mít stejné PHP (XML výstupy) i pro mobilní verzi 
    webu.
